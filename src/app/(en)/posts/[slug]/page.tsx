...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  try {
    ...
    const canonical = post.canonical_url || `${SITE_URL}/posts/${post.slug}`;
  ...
  return (
    <>
      ...
      <div className="max-w-5xl mx-auto py-4 md:py-8 px-3 md:px-4">
        <Breadcrumb
          items={[
            { label: "breadcrumb.posts", href: "/posts" },
            { label: post.title },
          ]}
        />
        <article>
          {post.cover_image && (
            <img
              ...
              {...(!post.cover_image.includes("/uploads/") && { loading: "lazy", style: { aspectRatio: "16/9" } })}
            />
          )}
